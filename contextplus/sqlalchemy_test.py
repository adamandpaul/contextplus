...
class TestDomainSQLAlchemyRecord(TestCase):
    ...
    def test_from_id(self):
        ...
        domain = sqlalchemy.DomainSQLAlchemyRecord.from_id.__func__(cls, parent, 'foo', id)
        ...
        cls.assert_called_with(parent=parent,
                               name='foo',
                               record=record)
...
class TestDomainSQLAlchemyRecordCollection(TestCase):
    ...
    def test_default_order_by_fields(self):
        ...
        self.assertEqual(collection.default_order_by_fields,
                         collection.child_type.id_fields)
    ...
    def test_child_from_record(self):
        ...
        collection.child_type.assert_called_with(parent=collection,
                                                 record=record)
    ...
    def test_filter(self):
        ...
        criteria = MagicMock()
        order_by = ['aaa']
        data = collection.filter(criteria=criteria,
                                 order_by=order_by,
                                 limit=10,
                                 offset=20)
        ...
        self.assertEqual(data['items'], [collection.child_from_record.return_value,
    ...
    def test_iter_children(self):
        ...
        collection.child_from_record.assert_any_call(r1)
        collection.child_from_record.assert_any_call(r2)
        self.assertEqual(children, [collection.child_from_record.return_value,
                                    collection.child_from_record.return_value])
